...
class ShapeGroupTest_Nonleaves_2Levels {
	...
	private static Object toPlainData(IntPoint[] points) {
		return Arrays.stream(points).map(p -> Map.of("x", p.getX(), "y", p.getY())).collect(Collectors.toList()); 
	}
	
	private static Object toPlainData(Color color) {
		return Map.of("red", color.getRed(), "green", color.getGreen(), "blue", color.getBlue());
	}
	
	private static Object toPlainData(RoundedPolygon polygon) {
		return Map.of(
				"vertices", toPlainData(polygon.getVertices()),
				"radius", polygon.getRadius(),
				"color", toPlainData(polygon.getColor()));
	}
	
	private static Object toPlainData(Extent extent) {
		return Map.of("left", extent.getLeft(), "top", extent.getTop(), "right", extent.getRight(), "bottom", extent.getBottom());
	}
	
	private static Object toPlainData(ShapeGroup shapeGroup) {
	...
	@Test
	void testExporter() {
		assert toPlainData(group3).equals(drawit.shapegroups1.exporter.ShapeGroupExporter.toPlainData(group3));
	}
